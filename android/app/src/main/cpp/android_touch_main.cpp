...
constexpr std::array<cd::EngineKind, 36> kEngines{{
    cd::EngineKind::macro, cd::EngineKind::body, cd::EngineKind::grain,
    cd::EngineKind::particle, cd::EngineKind::derelict_bed, cd::EngineKind::footsteps,
    cd::EngineKind::door, cd::EngineKind::pipe, cd::EngineKind::motor,
    cd::EngineKind::machinery, cd::EngineKind::crowd, cd::EngineKind::metal,
    cd::EngineKind::wind, cd::EngineKind::birds, cd::EngineKind::insects,
    cd::EngineKind::signal, cd::EngineKind::cave_air, cd::EngineKind::water_drip,
    cd::EngineKind::water_flow, cd::EngineKind::stone, cd::EngineKind::metro_traction,
    cd::EngineKind::rail_joint, cd::EngineKind::brake, cd::EngineKind::carriage,
    cd::EngineKind::music_box, cd::EngineKind::toy_voice, cd::EngineKind::toy_gears,
    cd::EngineKind::lullaby, cd::EngineKind::sub_drone, cd::EngineKind::tape_drone,
    cd::EngineKind::bowed_metal, cd::EngineKind::earth_rumble,
    cd::EngineKind::diagnostic, cd::EngineKind::plaits,
    cd::EngineKind::macro, cd::EngineKind::sub_drone,
}};

constexpr std::array<cd::EffectKind, 21> kEffects{{
...
constexpr std::array<cd::PlaitsModel, 16> kPlaitsModels{{
    cd::PlaitsModel::virtual_analog_vcf, cd::PlaitsModel::phase_distortion,
    cd::PlaitsModel::wave_terrain, cd::PlaitsModel::string_machine,
    cd::PlaitsModel::chiptune, cd::PlaitsModel::virtual_analog,
    cd::PlaitsModel::waveshaping, cd::PlaitsModel::fm, cd::PlaitsModel::grain,
    cd::PlaitsModel::additive, cd::PlaitsModel::wavetable, cd::PlaitsModel::chord,
    cd::PlaitsModel::swarm, cd::PlaitsModel::noise, cd::PlaitsModel::string,
    cd::PlaitsModel::modal,
}};

constexpr std::array<cd::PlaitsOutputMode, 4> kOutputs{{
    cd::PlaitsOutputMode::main, cd::PlaitsOutputMode::aux,
    cd::PlaitsOutputMode::mix, cd::PlaitsOutputMode::stereo,
}};

constexpr std::array<cd::ModWave, 4> kModWaves{{
    cd::ModWave::sine, cd::ModWave::triangle,
    cd::ModWave::sample_hold, cd::ModWave::random_walk,
}};

constexpr std::array<cd::ModDestination, 11> kDestinations{{
    cd::ModDestination::pitch, cd::ModDestination::timbre,
    cd::ModDestination::color, cd::ModDestination::motion,
    cd::ModDestination::texture, cd::ModDestination::level,
    cd::ModDestination::pan, cd::ModDestination::fx1,
    cd::ModDestination::fx2, cd::ModDestination::fx3,
    cd::ModDestination::fx4,
}};
...
enum class Action {
    ...
    scene_picker, engine_picker, actor_fx_select, actor_fx_picker,
...
enum class SliderKind {
    ...
    actor_level, actor_pan, tuning_root, euclidean_steps, euclidean_pulses,
...
int picker_count(PickerKind picker) {
    switch (picker) {
    case PickerKind::scene: return static_cast<int>(kScenes.size());
    case PickerKind::engine: return static_cast<int>(kEngines.size());
    case PickerKind::effect: return static_cast<int>(kEffects.size());
    case PickerKind::plaits_model: return static_cast<int>(kPlaitsModels.size());
    case PickerKind::output: return static_cast<int>(kOutputs.size());
    case PickerKind::scale: return static_cast<int>(g_scales.size());
    case PickerKind::mod_wave: return static_cast<int>(kModWaves.size());
    case PickerKind::mod_destination: return static_cast<int>(kDestinations.size());
...
std::string picker_label(PickerKind picker, int index, const cd::Session& session) {
    switch (picker) {
    case PickerKind::scene: return scene_name(kScenes[static_cast<std::size_t>(index)], ru(session));
    case PickerKind::engine: return engine_name(kEngines[static_cast<std::size_t>(index)], ru(session));
    case PickerKind::effect: return effect_name(kEffects[static_cast<std::size_t>(index)], ru(session));
    case PickerKind::plaits_model: return plaits_model_name(kPlaitsModels[static_cast<std::size_t>(index)]);
    case PickerKind::output: return output_name(kOutputs[static_cast<std::size_t>(index)]);
    case PickerKind::scale: return g_scales[static_cast<std::size_t>(index)].name;
    case PickerKind::mod_wave: return wave_name(kModWaves[static_cast<std::size_t>(index)], ru(session));
    case PickerKind::mod_destination: return destination_name(kDestinations[static_cast<std::size_t>(index)], ru(session));
...
int current_picker_index(const UiState& state, const cd::Session& session) {
    ...
    case PickerKind::scene:
        for (int i = 0; i < static_cast<int>(kScenes.size()); ++i) if (kScenes[static_cast<std::size_t>(i)] == session.scene) return i;
        break;
    case PickerKind::engine:
        for (int i = 0; i < static_cast<int>(kEngines.size()); ++i) if (kEngines[static_cast<std::size_t>(i)] == slot.engine) return i;
        break;
    case PickerKind::effect: {
        ...
        for (int i = 0; i < static_cast<int>(kEffects.size()); ++i) if (kEffects[static_cast<std::size_t>(i)] == kind) return i;
        break;
    }
...
void apply_picker_item(cd::Session& session, UiState& state, int index) {
    ...
    auto& mod = slot.modulators[static_cast<std::size_t>(state.modulator)];
    switch (state.picker) {
    case PickerKind::scene: cd::apply_scene_recipe(session, kScenes[static_cast<std::size_t>(index)]); break;
    case PickerKind::engine: slot.engine = kEngines[static_cast<std::size_t>(index)]; session.scene_modified = true; break;
    case PickerKind::effect:
        if (state.picker_master) session.master_effects[static_cast<std::size_t>(state.picker_effect)].kind = kEffects[static_cast<std::size_t>(index)];
        else slot.effects[static_cast<std::size_t>(state.picker_effect)].kind = kEffects[static_cast<std::size_t>(index)];
        session.scene_modified = true;
        break;
    case PickerKind::plaits_model: slot.plaits_model = kPlaitsModels[static_cast<std::size_t>(index)]; break;
    case PickerKind::output: slot.plaits_output = kOutputs[static_cast<std::size_t>(index)]; break;
    case PickerKind::scale: cd::apply_scale(slot.tuning, g_scales[static_cast<std::size_t>(index)]); break;
    case PickerKind::mod_wave: mod.wave = kModWaves[static_cast<std::size_t>(index)]; break;
    case PickerKind::mod_destination: mod.destination = kDestinations[static_cast<std::size_t>(index)]; break;
    case PickerKind::none: break;
    }
...
bool memory_exists(int slot) {
    std::error_code error;
    return std::filesystem::is_regular_file(g_memory_paths[static_cast<std::size_t>(slot)], error);
}
...
bool execute_action(cd::Session& session, UiState& state, const HitTarget& hit) {
    ...
    case Action::memory_load: {
        if (!memory_exists(state.memory_slot)) {
...
void draw_actor_events(SDL_Renderer* renderer, cd::Session& session, UiState& state,
    ...
    const int col_w = (area.w - gap) / 2;
    const int row_h = (area.h - gap * 3) / 4;
    button(renderer, state, {area.x, area.y, col_w, row_h},
        std::string{ru(session) ? "СОБЫТИЯ: " : "EVENTS: "} +
            (slot.euclidean.enabled ? (ru(session) ? "ВКЛ" : "ON") : (ru(session) ? "ВЫКЛ" : "OFF")),
        slot.euclidean.enabled, Action::euclidean_toggle, 0, 0, scale,
        kActorColors[static_cast<std::size_t>(state.actor)]);
    SDL_Rect scale_rect{area.x + col_w + gap, area.y, col_w, row_h};
    ...
    centered_text(renderer, scale_rect,
        ...
        kInk, scale);
    add_hit(state, scale_rect, Action::none);
    state.hits.back().action = Action::picker_close;
    state.hits.pop_back();
    add_hit(state, scale_rect, Action::none);
    state.hits.back().action = Action::scene_picker;
    state.hits.pop_back();
    add_hit(state, scale_rect, Action::none);
    state.hits.back().action = Action::engine_picker;
    state.hits.pop_back();
    add_hit(state, scale_rect, Action::none);
    state.hits.back().action = Action::actor_fx_picker;
    state.hits.pop_back();
    // Dedicated picker action is opened directly by touch handling below using a sentinel.
    add_hit(state, scale_rect, Action::actor_section, 99, 0);
    slider(renderer, state, {area.x, area.y + row_h + gap, col_w, row_h},
        ...
        scale, kActorColors[static_cast<std::size_t>(state.actor)]);
    slider(renderer, state, {area.x + col_w + gap, area.y + row_h + gap, col_w, row_h},
        ...
        scale, kActorColors[static_cast<std::size_t>(state.actor)]);
    slider(renderer, state, {area.x, area.y + 2 * (row_h + gap), col_w, row_h},
        ...
        SliderKind::euclidean_pulses, 0, 0, scale,
        kActorColors[static_cast<std::size_t>(state.actor)]);
    slider(renderer, state, {area.x + col_w + gap, area.y + 2 * (row_h + gap), col_w, row_h},
        ...
        SliderKind::euclidean_rotation, 0, 0, scale,
        kActorColors[static_cast<std::size_t>(state.actor)]);
    slider(renderer, state, {area.x, area.y + 3 * (row_h + gap), col_w, row_h},
        ru(session) ? "ВЕРОЯТНОСТЬ" : "PROBABILITY", percent(slot.euclidean.probability),
        slot.euclidean.probability, SliderKind::euclidean_probability, 0, 0, scale,
        kActorColors[static_cast<std::size_t>(state.actor)]);
    if (slot.engine == cd::EngineKind::plaits) {
        SDL_Rect model_rect{area.x + col_w + gap, area.y + 3 * (row_h + gap), col_w, row_h / 2 - gap / 2};
        ...
        SDL_Rect output_rect{model_rect.x, model_rect.y + model_rect.h + gap, model_rect.w, row_h - model_rect.h - gap};
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        button(renderer, state, rect,
            std::string{ru(session) ? "СЛОТ " : "SLOT "} + std::to_string(i + 1) +
                (memory_exists(i) ? (ru(session) ? "  СОХРАНЕН" : "  SAVED") : (ru(session) ? "  ПУСТ" : "  EMPTY")),
            i == state.memory_slot, Action::memory_select, i, 0, scale,
            memory_exists(i) ? kGreen : kPurple);
    }
...
extern "C" int SDL_main(int argc, char** argv) {
    ...
    desired.channels = 2;
    desired.samples = static_cast<Uint16>(next_power_of_two(std::max(2048, burst * 6)));
