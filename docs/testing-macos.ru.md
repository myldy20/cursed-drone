...
| открыть выбор ландшафта / движка / FX и подтвердить | E |
...
На экране `Сцена` клавиша E открывает выбор из шести ландшафтов: `Заброшенное / Цех / Пустошь / Мокрая пещера / Вагон метро / Сломанная детская`. E также подтверждает выбор. Пять макросов — `Материал / Активность / Напряжение / Дистанция / Развитие`; Down после `Развития` переводит фокус на дорожки, где A/D меняет уровень выбранной. `Слоты` показывает четыре актёра текущей сцены и их осмысленные параметры. На FX видны все четыре исходные дорожки, S выбирает одну из них. Справа сверху вместо повтора названия экрана показываются DSP load и уровень master-выхода. Изменения автоматически сохраняются примерно через 0,75 с бездействия.
