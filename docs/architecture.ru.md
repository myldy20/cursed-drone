...
В `0.4.0` scene recipe назначает четырём слотам разные роли и безопасные стартовые диапазоны. `Заброшенное` использует фон/шаги/дверь/трубу, `Цех` — мотор/станок/толпу/металл, `Пустошь` — ветер/птиц/насекомых/сигнал. Прежние `TONE`, `RESONATOR`, `GRAINLET` и `PARTICLES` остаются доступными вручную. Все источники реализуют тот же контракт:
...
Над детальными настройками лежит `PerformanceSettings`. Хранимые поля `texture`, `pulse`, `chaos`, `space`, `events` в 0.4 получают продуктовый смысл `Материал`, `Активность`, `Напряжение`, `Дистанция`, `Развитие`; `fade` остаётся отдельным состоянием мастер-выхода. Schema 4 также хранит scene и разные времена fade-in/fade-out. Макросы вычисляются внутри callback поверх базовых значений. Loader принимает schema 1–3 и переводит старый стартовый патч в безопасную сцену `Заброшенное`.
