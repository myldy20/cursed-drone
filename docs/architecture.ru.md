...
В `0.1.0` доступен диагностический движок, чтобы проверить тракт. Он не является попыткой заменить Plaits, Elements, Rings, Clouds или DaisySP. Продуктовые движки должны реализовать одинаковый контракт:
...
Все непрерывные параметры сглаживаются однополюсным фильтром примерно за 20 мс; master — за 30 мс. Переключатели и типы эффектов меняются на границе блока.
...
Следующие источники: envelope follower, Brownian motion, logistic chaos, Euclidean pulse, probability burst и короткая step curve. Они будут синхронизироваться общим clock, но не создадут трекерную сетку.
...
Позже добавится пользовательская команда `Panic`: короткий fade, сброс feedback/freeze и безопасное восстановление. Она должна работать даже при перегрузке UI.
...
Логическое разрешение — `512×384`. На Brick оно масштабируется ровно в 2 раза до `1024×768`; на широких дисплеях используется letterbox. Цвет и геометрия не несут единственный смысл состояния: финальный UI добавит текст, форму и focus outline.
