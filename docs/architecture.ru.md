...
Scene recipe назначает четырём слотам разные роли и безопасные стартовые диапазоны. В 0.6 к `Заброшенному`, `Цеху` и `Пустоши` добавлены `Мокрая пещера`, `Вагон метро` и `Сломанная детская`. Прежние `TONE`, `RESONATOR`, `GRAINLET` и `PARTICLES` остаются доступными вручную. Все источники реализуют тот же контракт:
...
Над детальными настройками лежит `PerformanceSettings`. Хранимые поля `texture`, `pulse`, `chaos`, `space`, `events` имеют продуктовый смысл `Материал`, `Активность`, `Напряжение`, `Дистанция`, `Развитие`; `fade` остаётся отдельным состоянием мастер-выхода. Schema 6 хранит шесть типов сцен, расширенные engine/FX-enum и разные времена fade-in/fade-out. Loader сохраняет чтение schema 1–5.
...
Актёры 0.4 имеют собственные планировщики: короткие события, составные heel/toe и impact/exhaust, кластеры, большие циклы приближения и серии с bounded-random паузой. Общий tempo задаёт масштаб повторяющихся процессов, но не создаёт tracker transport. Детерминированные PRNG-потоки разделены по слотам, а направленные контуры не подменяются случайным LFO.
...
2. мягкий `tanh` limiter;
3. сглаженный master/fade gain после лимитера.

Так весь диапазон 0–100% остаётся слышимым и не сплющивается сатурацией, а финальный выход всё равно находится внутри `[-1, 1]`.
